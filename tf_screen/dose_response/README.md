...
* **Topic-level dose-response.** Topic probabilities are also aggregated into the metacell `.obs` (`prob_topic_<Topic_N>` columns) by `build_metacells.py`. A topic-level correlation analysis can be obtained by editing `correlation_analysis.py`'s `Config.COARSE_STATES` to point at the topic columns, but no separate script is shipped as the paper does not feature topic-level dose-response in Fig. 3.
